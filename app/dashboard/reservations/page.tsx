...
export default function DashboardReservations() {
  ...
  const load = async () => {
    const { data } = await supabase
      .from('reservations')
      .select('*, events(title, date, location, price, image_url)')
        ...
        <div style={{ backgroundColor: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: '20px', padding: '32px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '28px', flexWrap: 'wrap', gap: '16px' }}>
            ...
            <div style={{
              ...
              padding: '8px 20px', borderRadius: '999px', fontSize: '11px', fontWeight: 700, letterSpacing: '2px'
            }}>
            ...
            {[
              ...
              return (
                <div key={step.key} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div style={{
                    padding: '5px 12px', borderRadius: '999px', fontSize: '10px', fontWeight: 700, letterSpacing: '1px',
                    backgroundColor: isCurrent ? `${statusColors[step.key]}20` : isDone ? 'rgba(16,185,129,0.1)' : '#111',
                    border: `1px solid ${isCurrent ? statusColors[step.key] : isDone ? 'rgba(16,185,129,0.4)' : '#1a1a1a'}`,
                    color: isCurrent ? statusColors[step.key] : isDone ? '#10b981' : '#333',
                  }}>
                    {isDone ? '✓ ' : ''}{step.label}
                  </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '24px' }}>
            {[
              ...
              { label: 'TICKETS', value: `${selected.num_people} person${selected.num_people > 1 ? 's' : ''}` },
              { label: 'TOTAL', value: `${selected.total_price} EGP` },
              { label: 'EVENT DATE', value: new Date(selected.events?.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) },
              { label: 'BOOKED ON', value: new Date(selected.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) },
            ].map(item => (
              <div key={item.label} style={{ backgroundColor: '#111', border: '1px solid #1a1a1a', borderRadius: '10px', padding: '14px 16px' }}>
                <p style={{ color: '#333', fontSize: '10px', letterSpacing: '2px', fontWeight: 700, margin: '0 0 4px' }}>{item.label}</p>
          ...
          {/* Additional Guests */}
          {selected.people_details?.length > 0 && (
            <div style={{ backgroundColor: '#111', border: '1px solid #1a1a1a', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
              <p style={{ color: '#333', fontSize: '10px', letterSpacing: '2px', fontWeight: 700, margin: '0 0 16px' }}>ADDITIONAL GUESTS ({selected.people_details.length})</p>
              {selected.people_details.map((p: any, i: number) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', backgroundColor: '#0d0d0d', borderRadius: '8px', padding: '12px', marginBottom: '8px' }}>
                  <div><p style={{ color: '#333', fontSize: '9px', letterSpacing: '1px', margin: '0 0 2px' }}>NAME</p><p style={{ color: '#fff', fontSize: '13px', margin: 0 }}>{p.name}</p></div>
                  <div><p style={{ color: '#333', fontSize: '9px', letterSpacing: '1px', margin: '0 0 2px' }}>PHONE</p><p style={{ color: '#fff', fontSize: '13px', margin: 0 }}>{p.phone}</p></div>
                  <div><p style={{ color: '#333', fontSize: '9px', letterSpacing: '1px', margin: '0 0 2px' }}>INSTAGRAM</p><p style={{ color: '#fff', fontSize: '13px', margin: 0 }}>@{p.instagram}</p></div>
                </div>
              ))}
            </div>
          )}
  ...
  <div style={{ backgroundColor: '#111', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '12px', padding: '16px 20px', marginBottom: '24px' }}>
    ...
    <p style={{ color: '#3b82f6', fontSize: '22px', fontWeight: 900, fontFamily: 'monospace', letterSpacing: '3px', margin: 0 }}>
      {selected.payment_sender_phone}
          ...
          {/* Entry Code */}
          {selected.entry_code && (
            <div style={{ backgroundColor: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '12px', padding: '20px', textAlign: 'center', marginBottom: '24px' }}>
              <p style={{ color: '#444', fontSize: '10px', letterSpacing: '3px', margin: '0 0 8px' }}>ENTRY CODE</p>
              <p style={{ color: '#10b981', fontSize: '36px', fontWeight: 900, letterSpacing: '8px', fontFamily: 'monospace', margin: 0 }}>{selected.entry_code}</p>
            </div>
          )}
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '24px' }}>
          {['all', ...allStatuses].map(s => (
            ...
            >
              {s.replace(/_/g, ' ').toUpperCase()} ({s === 'all' ? reservations.length : reservations.filter(r => r.status === s).length})
            </button>
          ))}
        </div>
